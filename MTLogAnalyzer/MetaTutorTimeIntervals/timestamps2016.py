...
for index in range(data.shape[0]):
    if currentid == -1 or (int(data['RawPID'][index][-3:]) != currentid and currentid > -1): #new student in logs
        currentid = int(data['RawPID'][index][-3:])
        EIV_count = 1
    if isnan(data["date"][index]) or isnan(data["logTime"][index]):
        sc_ids.append("NA")
    else:
        EIVtime = int(calendar.timegm(datetime.datetime.strptime(data["date"][index]+" "+data["logTime"][index], "%d/%m/%Y %H:%M:%S").timetuple()) * 1000000 )
...
for index in range(data.shape[0]):
    ...
    if data['eiv_count'][index] > 1:
        ...
        times_interval[data['eiv_count'][index]][data['RawPID'][index]]=(time_prev_eiv, time_curr_eiv)
