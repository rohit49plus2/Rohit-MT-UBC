...
import datetime
import time
from MetaTutorTimeIntervals.Events import *
...
class MTLogParser(object):
    ...
    def getIdxOfLastListEltWithPattInPos(self, logger, l, patt, pos=0, reportError=True):
        ...
        l.reverse()
        try:
            idx = map((lambda x: x[pos]), l).index(patt)
        except IndexError:
            if reportError:
    ...
    def isInListEltWithPattInPos(self, l, patt, pos=0):
        ...
        try:
            map((lambda x: x[pos]), l).index(patt)
        except ValueError:
            return False
        else:
            return True
    ...
    def parseDay2LogEvents(self, logger, mtsubject, matPageSubgoalDict, possibleSubgoalsNamesID, stopTimeStamp=None, startTimeStamp=None):
        ...
        """if stopTimeStamp is not None:
            if not isinstance(stopTimeStamp, datetime.datetime):
                stopTimeStamp = Event.Event.convertTimeMT2Standard(stopTimeStamp)
        if startTimeStamp is not None:
            if not isinstance(startTimeStamp, datetime.datetime):
                startTimeStamp = Event.Event.convertTimeMT2Standard(startTimeStamp)"""      
        ...
        for iEv, event in enumerate(mtsubject.day2Events):
            """ NJ TESTED: assert(event.getTimeStart() == event.getTimeEnd())
            found that events have only one time stamp (start always corresponds to end)
            which means I can do this:"""
            """if stopTimeStamp != None and event.timestamp >= stopTimeStamp and initialSubgoalsSet and firstCEvtPursuingNewSubgoalAppended: # don't exit loop until first two subgoals are set 
                print "BREAKING PARSE DAY 2 LOOP EARLY"
                time.sleep(5)
                break
            
            if debug:
                print "stopTimeStamp:", stopTimeStamp
                print "startTimeStamp:", startTimeStamp
                print "Event Time stamp:", event.timestamp"""
...
#                    #---------
                ...
                if True in map(lambda evtClass:isinstance(event, evtClass), [Rule.MTRuleMPTGEvent, Rule.MTRuleJOLEvent,
        ...
        """NJ ADDED: NEED TO PERFORM CLEANUP ON ALL QUEUED EVENTS IF LOOP EXITS EARLY BECAUSE OF STOPTIMESTAMP"""
        """Type of queued events: CEvtInitialSubgoalsSetting, MetaTutor.Events.Rule.MTRulePLANEvent object, CEvtQuestionnaireOngoing"""
