...
for data in datasets:
    ...
    f = open(dir_path+'/results_smote/'+ep[0]+'_'+ep[1]+'/'+folder+'/Ensemble'+result_suffix+'_'+ep[0]+'_'+ep[1]+'_'+data+'.txt', 'w')
    ...
    m1 = AdaBoostClassifier(lr)
    m2  = RandomForestClassifier()

    parameters = {'lr__n_estimators':range(1, 2, 3),
    'rf__max_depth':[1,2,3,4,5],
    'rf__n_estimators': [10,50,100],
    'rf__max_features': [1,2,3,4,5]
    }
    model = VotingClassifier(estimators=[('lr', m1), ('rf', m2)], voting='soft')
    ...
    dict_results={'Model':'Ensemble_SMOTE','majority_baseline_accuracy':accuracy1,'stratified_baseline_accuracy':accuracy2 ,'mean_accuracy':np.mean(scores), 'std_dev_accuracy':np.std(scores), 'mean_confusion_matrix':mean_of_conf_matrix_arrays,'confusion_matrices':conf_matrix_list_of_arrays}

    with open(dir_path+'/results_smote/'+ep[0]+'_'+ep[1]+'/'+folder+'/Ensemble'+result_suffix+'_'+ep[0]+'_'+ep[1]+'_'+data+'.pickle', 'wb') as handle:
