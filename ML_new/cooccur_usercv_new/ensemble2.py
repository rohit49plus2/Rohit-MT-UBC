...
Xlog = combined[log_c].drop(emotions,axis=1)
Xlog=Xlog.drop('Unnamed: 0',axis=1)

Xeye = combined[eye_c].drop(emotions,axis=1)
Xeye=Xeye.drop('Unnamed: 0',axis=1)
...
for i in range(10):
    for fold_ind, (train_index, test_index) in enumerate(stratified_group_k_fold(Xlog, y, ids, k=8)):
        ...
        pca = IncrementalPCA(n_components=Xlog_train.shape[1]//5, batch_size=120)
        pca.fit(Xlog_train)
        ...
        pca = IncrementalPCA(n_components=Xeye_train.shape[1]//5, batch_size=120)
        pca.fit(Xeye_train)
        ...
        clf2 = GridSearchCV(svm, parameters_svm,cv=5, n_jobs=4)
        ...
        clf4 = GridSearchCV(svm, parameters_svm,cv=5, n_jobs=4)
...
dict_results={'Model':'ENSEMBLE2_SMOTE','mean_accuracy':np.mean(scores), 'std_dev_accuracy':np.std(scores), 'mean_confusion_matrix':mean_of_conf_matrix_arrays,'confusion_matrices':conf_matrix_list_of_arrays}
