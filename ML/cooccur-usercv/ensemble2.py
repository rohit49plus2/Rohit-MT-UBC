...
for i in range(10):
    for fold_ind, (train_index, test_index) in enumerate(stratified_group_k_fold(Xlog, y, ids, k=8)):
        ...
        ipca = IncrementalPCA(n_components=Xlog_train.shape[1]//5, batch_size=120)
        ipca.fit(Xlog_train)
        Xlog_train=ipca.transform(Xlog_train)
        Xlog_test=ipca.transform(Xlog_test)

        Xlog_train, ylog_train = pipeline.fit_resample(Xlog_train, y_train)#Smote
        # print(data,X_train.shape)

        ipca = IncrementalPCA(n_components=Xeye_train.shape[1]//5, batch_size=120)
        ipca.fit(Xeye_train)
        Xeye_train=ipca.transform(Xeye_train)
        Xeye_test=ipca.transform(Xeye_test)
        ...
        pred_prob = (pred1 + pred2 + pred3 + pred4)/4
