...
for N in [5,10,15,20]:
    ...
    df = df.set_index('id')
    # plt.plot(df['Score'],df['Match Score'])
    plt.plot(df['Score'],df['Mean Position Difference'])
    plt.show()
    # print(df)
    # print(df[['id','Tobii Fixations','New Fixations']].to_latex())
    # print(df[['id','Mean Position Difference','StdDev Position Difference']].round(2).to_latex())
    # print(df[['id','Mean Duration Difference','StdDev Duration Difference']].round(2).to_latex())
    # print(df[['id','Match Score']].round(2).to_latex())
