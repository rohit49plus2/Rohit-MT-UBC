...
out = paste(dirout, "predic_MT_COOC_BinaryBothVsNone_stratifiedSmote.csv", sep="")
...
outwithinfolds = paste(dirout, "predic_MT_COOC_BinaryBothVsNone_stratifiedSmote_withinfolds.csv", sep="")
...
outconfmatrix = paste(dirout, "predic_MT_COOC_BinaryBothVsNone_stratifiedSmote_confusionmatrix.txt", sep="")
#outwithinfolds = paste(dirout, "predic_MT_valencepredbinarylumpednegative_withinfolds.csv", sep="")
#outwithinfolds = paste(dirout, "predic_MT_anxious_withinfolds.csv", sep="")
sink(outconfmatrix)
sink()
...
for(dataset_i in datasets){
  ...
  data$Class[data$Class == "Curiosity"] = "None"
  data$Class[data$Class == "Anxiety"] = "None"
  data$Class[data$Class == "Boredom"] = "None"
  data$Class[data$Class == "Frustration"] = "None"
  ...
  for (run_i in 1:runs) #multiple runs of CV
  {
    ...
    for(i in seq_len(ms)) {	#K-folds cross-validation
      ...
      if(feat_selection){
        ...
        #print(traindata[, colnames(traindata) != dependant])
      ...
      #SMOTE train data if not balanced
      nbC1 = summary(traindata[, dependant])[1]
      nbC2 = summary(traindata[, dependant])[2]
      
      if( nbC1 != nbC2 && (abs(nbC1 - nbC2) / (nbC1+nbC2)) > .05 ){
        percover = (abs(nbC1 - nbC2) * 100 / min(nbC1, nbC2))
        num_synthetic = min(nbC1, nbC2) * percover / 100
        percunder = max(nbC1, nbC2) / num_synthetic * 100
        traindata$Class = as.factor(traindata$Class)
        traindata = na.exclude(SMOTE(fmla, traindata, perc.over = percover, perc.under = percunder))
      }	
      
      train_baseC1 = summary(traindata[, dependant])[1] / nrow(traindata)
      train_baseC2 = summary(traindata[, dependant])[2] / nrow(traindata)
      
      #print (paste("dataset", dataset_i, " run ", run_i, " fold ", i, ": distrib test set = ", fold_baseC1, " / ", fold_baseC2, sep=""))
      print (paste("dataset", dataset_i, " run ", run_i, " fold ", i, ": distrib train set = ", train_baseC1, " / ", train_baseC2, sep=""))
      ...
      #baseline
      base = max(summary(data[, dependant])[1], summary(data[, dependant])[2]) / nrow(data)
      #base = max(summary(data[, dependant])[1], summary(data[, dependant])[2], summary(data[, dependant])[3], summary(data[, dependant])[4]) / nrow(data)
      withinfoldbase = max(summary(testdata[, dependant])[1], summary(testdata[, dependant])[2]) / nrow(testdata)
      ...
      # sensiLB = sensiLB + c(resCM$byClass[1,1], resCM$byClass[2,1], resCM$byClass[3,1])
      # speciLB = speciLB + c(resCM$byClass[1,2], resCM$byClass[2,2], resCM$byClass[3,2])
      # F1LB = F1LB + c(resCM$byClass[1,7], resCM$byClass[2,7], resCM$byClass[3,7])
      ...
      # sensiRF = sensiRF + c(resCM$byClass[1,1], resCM$byClass[2,1], resCM$byClass[3,1])
      # speciRF = speciRF + c(resCM$byClass[1,2], resCM$byClass[2,2], resCM$byClass[3,2])
      # F1RF = F1RF + c(resCM$byClass[1,7], resCM$byClass[2,7], resCM$byClass[3,7])
      ...
      # sensiNN = sensiNN + c(resCM$byClass[1,1], resCM$byClass[2,1], resCM$byClass[3,1])
      # speciNN = speciNN + c(resCM$byClass[1,2], resCM$byClass[2,2], resCM$byClass[3,2])
      # F1NN = F1NN + c(resCM$byClass[1,7], resCM$byClass[2,7], resCM$byClass[3,7])
      ...
      # sensiSVM = sensiSVM + c(resCM$byClass[1,1], resCM$byClass[2,1], resCM$byClass[3,1])
      # speciSVM = speciSVM + c(resCM$byClass[1,2], resCM$byClass[2,2], resCM$byClass[3,2])
      # F1SVM = F1SVM + c(resCM$byClass[1,7], resCM$byClass[2,7], resCM$byClass[3,7])
